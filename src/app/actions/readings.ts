...
import { readingFormSchema, type ReadingFormValues } from './readings.schema';
...
export async function createReading(formData: ReadingFormValues) {
  ...
  try {
    ...
    // Recalc runs after the insert commits. If it throws, the reading is still
    // saved; surface recalcFailed so the caller can warn the summary is stale.
    let recalcFailed = false;
    try {
      await triggerRecalculation(data.buildingId);
    } catch (err) {
      recalcFailed = true;
      console.error('Recalculation failed for building ' + data.buildingId + ':', err);
    }
    ...
    return { success: true, reading, recalcFailed };
  } catch (error) {
...
export async function deleteReading(id: string, buildingId: string) {
  ...
  try {
    ...
    let recalcFailed = false;
    try {
      await triggerRecalculation(buildingId);
    } catch (err) {
      recalcFailed = true;
      console.error('Recalculation failed for building ' + buildingId + ':', err);
    }
    ...
    return { success: true, recalcFailed };
  } catch (error) {
